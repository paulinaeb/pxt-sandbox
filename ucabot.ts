 ...
 */
  ...
  */ 
   function sendMsg(d: string, c: string, p: string[], req: boolean, stop: number, n_times: number): boolean {
 ...
 */
  ...
  export function nearMe(d: number): string { 
    for (let i = 0; i < 6; i++){
      if (sendMsg('0', 'WN', [d.toString()], true, 0, 25)){  
 ...
 */
  ...
  export function followAgent(id: number) {
    ...
    if (id_followed == id_agent)
      ...
      for (let i = 0; i < 3; i++){
        if (sendMsg('0', 'AE', [id_followed], true, 0, 30)){ 
